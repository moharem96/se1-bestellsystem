...
public class Runtime {
    ...
    public static void main(String[] args) {
        ...
        instance.run(); // invoke {@code run()} method (of {@link Runnable} interface) on created
                        // instance
    }
     ...
     * @param classNames list of fully-qualified class names from which instances
     *                   are attempted to be created
     * @param assignable functional interface to test class of instance to create is
     *                   assignable from {@code T}
     * @param properties properties from application.properties file passed to
     *                   created instance with constructor
     * @param args       command line arguments passed to created instance with
     *                   constructor
     * @param alt        alternative instance supplier when creation from
     *                   {@code classNames} yields no result
    ...
    Properties loadPropertiesFromFile(String fileName) {
        ...
        try (InputStream is = List.of(fileName, "resources/" + fileName).stream()
                ...
                .findFirst().map(opt -> opt.get()).orElse(new /* empty */InputStream() {
                    public int read() {
                        return -1;
                    }
                });) {
